...
def scale_and_center_point_cloud(point_cloud, target_size=2.0):
    # Calculate the bounding box dimensions for each dimension
    ...
    # Calculate the scale factor for each dimension (assuming isotropic scaling)
    scale_factors = target_size / np.max(bbox_dims)

    # Scale the point cloud along each dimension
    scaled_and_centered_point_cloud = translated_point_cloud * scale_factors
...
if __name__ == "__main__":
    ...
    with open('Data/4d_scaled_centered_shapes_data.pkl', 'wb') as file:
